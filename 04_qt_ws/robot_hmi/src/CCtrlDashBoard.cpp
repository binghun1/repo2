...
CCtrlDashBoard::CCtrlDashBoard(QWidget *parent,double max_kd, bool is_v_flag, StyleType type) :
    QWidget(parent),
    m_StyleType(type),
    max_kd(max_kd),
    is_v_flag(is_v_flag)
...
{
    qreal tSteps = (100- m_MinValue)/m_DashNum; //相乘后的值是分的份数
...
void CCtrlDashBoard::drawIndicator(QPainter *painter, qreal hlafWidth)
{
    ...
    if(m_DashValue>0){
        qreal tSteps = (100 - m_MinValue)/m_DashNum; //相乘后的值是分的份数
        qreal angleStep = 1.0*(360.0-m_StartAngle - m_EndAngle) / tSteps; //每一个份数的角度
        double degRotate = angleStep*tSteps*(m_DashValue/m_MaxValue);
...
void CCtrlDashBoard::drawIndicatorBar(QPainter *painter, qreal hlafWidth)
{
    ...
    if(m_DashValue>0){
        qreal tSteps = (100 - m_MinValue)/m_DashNum; //相乘后的值是分的份数
        qreal angleStep = 1.0*(360.0-m_StartAngle - m_EndAngle) / tSteps; //每一个份数的角度
        degRotate = angleStep*tSteps*(m_DashValue/m_MaxValue);
